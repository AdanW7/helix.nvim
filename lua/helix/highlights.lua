...
hl.common = {
    ...
    MatchParen = {bg = c.lavendar, fg = c.black},
    NonText = {fg = c.grey},
...
function M.setup()
    ...
    for group_name, group_settings in pairs(cfg.highlights) do
        local fg = group_settings.fg and (group_settings.fg:sub(1, 1) == '$' and c[group_settings.fg:sub(2)] or group_settings.fg) or "none"
        local bg = group_settings.bg and (group_settings.bg:sub(1, 1) == '$' and c[group_settings.bg:sub(2)] or group_settings.bg) or "none"
        local sp = group_settings.sp and (group_settings.sp:sub(1, 1) == '$' and c[group_settings.sp:sub(2)] or group_settings.sp) or "none"
